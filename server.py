...
async def tcplink(sock, addr):
    ...
    while True:
        ...
        if data == b'exit':
            break
        try:
            name = (((str(data).split(':'))[1]).split('\\n'))[0]
            passwd = (((str(data).split(':'))[2]).split('\\n'))[0]
        except IndexError:
            pass
        ...
        # print tmp
        if tmp:
            await loop.sock_sendall(sock,b"error:the username already had")
        else:
            db.execute("INSERT INTO INFO(user,passwd) \
                       VALUES('%s','%s')" % (name, passwd))
            t.commit()
            await loop.sock_sendall(sock,b"register complete!the name is %s,the passwd is %s" % (name, passwd))
